    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-xl shadow-md p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-display font-bold text-dark-900">{{ number_format($stats['total_spent']) }} <span class="text-sm">FCFA</span></p>
                    </div>
        ...
        <div class="bg-gradient-to-r from-primary-600 to-primary-700 rounded-xl shadow-md p-8 mb-8">
            <div class="flex items-center justify-between">
                ...
                <a href="{{ route('agencies.index') }}" class="bg-white hover:bg-gray-100 text-primary-700 px-8 py-3 rounded-lg font-semibold transition-all transform hover:scale-105 flex items-center">
        ...
        <div class="mb-6">
            <div class="border-b border-dark-200">
                <nav class="-mb-px flex space-x-8">
                    <button class="tab-btn active border-b-2 border-primary-600 py-4 px-1 text-sm font-semibold text-primary-600" data-tab="upcoming">
                        Upcoming Trips
                    </button>
                    <button class="tab-btn border-b-2 border-transparent py-4 px-1 text-sm font-semibold text-dark-500 hover:text-dark-700 hover:border-dark-300" data-tab="past">
                        Past Trips
                    </button>
                    <button class="tab-btn border-b-2 border-transparent py-4 px-1 text-sm font-semibold text-dark-500 hover:text-dark-700 hover:border-dark-300" data-tab="cancelled">
                        Cancelled
                    </button>
                </nav>
            </div>
        ...
        <div id="upcoming-tab" class="tab-content">
            <div class="space-y-4">
                    ...
                    <div class="p-6">
                        <div class="flex items-start justify-between mb-4">
                            <div class="flex items-start space-x-4">
                                @if($booking->agency->logo)
                                    <img src="{{ asset('storage/' . $booking->agency->logo) }}" alt="{{ $booking->agency->name }}" class="h-12 w-12 object-contain">
                                @else
                                    <div class="w-12 h-12 bg-primary-100 rounded-lg flex items-center justify-center">
                                        <span class="text-lg font-display font-bold text-primary-600">{{ substr($booking->agency->name, 0, 1) }}</span>
                                    </div>
                                @endif
                                <div>
                        ...
                        <div class="grid md:grid-cols-4 gap-6 mb-6">
                            ...
                            <div>
                                ...
                                <p class="font-semibold text-dark-900">{{ date('M d, Y', strtotime($booking->travel_date)) }}</p>
                                <p class="text-sm text-dark-600">{{ date('g:i A', strtotime($booking->departure_time)) }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-dark-500 mb-1">Seats</p>
                                <div class="flex flex-wrap gap-1">
                                    @foreach(json_decode($booking->seats) as $seat)
                                    <span class="bg-primary-100 text-primary-700 px-2 py-0.5 rounded text-xs font-semibold">{{ $seat }}</span>
                                    @endforeach
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between pt-4 border-t border-dark-200">
                            <div>
                                <p class="text-2xl font-bold text-dark-900">{{ number_format($booking->total_amount) }} FCFA</p>
                                <p class="text-sm text-dark-500">{{ count(json_decode($booking->seats)) }} seat(s)</p>
                            </div>
                            <div class="flex space-x-3">
                                <a href="{{ route('bookings.ticket', $booking->id) }}" class="bg-primary-600 hover:bg-primary-700 text-white px-6 py-2 rounded-lg font-semibold transition-all flex items-center">
                                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z"/>
                                    </svg>
                                    View Ticket
                                </a>
                                <button onclick="cancelBooking({{ $booking->id }})" class="border border-red-300 text-red-600 hover:bg-red-50 px-6 py-2 rounded-lg font-semibold transition-all">
                                    Cancel
                                </button>
                            </div>
                        </div>
                ...
                <div class="bg-white rounded-xl shadow-md p-12 text-center">
                    ...
                    <a href="{{ route('agencies.index') }}" class="inline-flex items-center bg-primary-600 hover:bg-primary-700 text-white px-6 py-3 rounded-lg font-semibold transition-all">
                ...
                <div class="bg-white rounded-xl shadow-md overflow-hidden opacity-90">
                    <div class="p-6">
                        <div class="flex items-start justify-between mb-4">
                            ...
                            <span class="bg-gray-100 text-gray-700 px-3 py-1 rounded-full text-sm font-semibold">Completed</span>
                        </div>
                        ...
                        <div class="grid md:grid-cols-4 gap-6">
                            ...
                            <div>
                                ...
                                <p class="text-xl font-bold text-dark-900">{{ number_format($booking->total_amount) }} FCFA</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-md overflow-hidden opacity-75">
                    <div class="p-6">
                        <div class="flex items-start justify-between mb-4">
                            <div class="flex items-start space-x-4">
                                @if($booking->agency->logo)
                                    <img src="{{ asset('storage/' . $booking->agency->logo) }}" alt="{{ $booking->agency->name }}" class="h-12 w-12 object-contain grayscale">
                                @else
                                    <div class="w-12 h-12 bg-red-100 rounded-lg flex items-center justify-center">
                                        <span class="text-lg font-display font-bold text-red-600">{{ substr($booking->agency->name, 0, 1) }}</span>
                                    </div>
                                @endif
                                <div>
                                    <h3 class="text-lg font-bold text-dark-900">{{ $booking->agency->name }}</h3>
                        ...
                        <div class="grid md:grid-cols-4 gap-6">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-semibold text-green-600">{{ $booking->refund_status ?? 'Processed' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
